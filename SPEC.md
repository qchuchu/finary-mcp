...
Categorize a transaction (conversational):
1. (Optionally) list categories to find the target category id.
2. Categorize a transaction by id.
...
**Tool: categorize-transaction** — writes, reversible
- Input: `{ transactionId: number, categoryId: number }`
...
- `PATCH {ctx}/transactions/{id}` body `{ "category_id": <id> }` → `{ result: Transaction }`
